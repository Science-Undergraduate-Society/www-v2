import { BlueCardPartner } from "@/lib/types";

export const blueCardPartners: BlueCardPartner[] = [
  {
    name: "Susgrainable",
    logoSrc: "/assets/bluecard-images/partner-logos/susgrainable.png",
    description: "Use code BLUECARD for 20% off online orders.",
    location: "Valid at https://susgrainable.com/",
    website: "https://susgrainable.com/",
  },
  {
    name: "Prestige Optical",
    logoSrc: "/assets/bluecard-images/partner-logos/prestigeOptical.jpg",
    description: "10% discount for all contact lenses.",
    location: "2950 West Broadway, Vancouver, BC V6K 2G8",
    website: "https://www.prestigeoptical.ca/",
  },
  {
    name: "Pita Pit",
    logoSrc: "/assets/bluecard-images/partner-logos/pitaPit.jpeg",
    description: "Get 10% off menu items.",
    location: "5717 Dalhousie Rd, Vancouver, BC V6T 2H9",
    website: "https://pitapit.ca/",
  },
  {
    name: "Chatime",
    logoSrc: "/assets/bluecard-images/partner-logos/chatime-logo.webp",
    description: "Get 10% off all products.",
    location: "Valid only at 5998 University Blvd #4, Vancouver, V6T 2A1",
    website: "https://chatime.ca/",
  },
  {
    name: "Eyelab, Doctors of Optometry",
    logoSrc: "/assets/bluecard-images/partner-logos/EYELAB.png",
    description: "Get 10% off all products.",
    location: "Valid only at 2369 W 41st Ave, Vancouver, BC V6M 2A3",
    website: "https://helloeyelab.com/",
  },
  {
    name: "Killer Ice Cream",
    logoSrc: "/assets/bluecard-images/partner-logos/Killer-ice-cream.png",
    description: "Get $2 off regular sized cones.",
    location: "Valid only at 3659 W 4th Ave, Vancouver, BC V6R 1P2",
    website: "https://killericecream.ca/",
  },
  {
    name: "Majestic Maven",
    logoSrc: "/assets/bluecard-images/partner-logos/majestic-maven-14.jpg",
    description: "Get 20% off all lash and brow services.",
    location: "Valid only at 1236 Richards St, Vancouver, BC V6B 6M6",
    website: "https://majesticmaven.ca/",
  },
  {
    name: "Mosa",
    logoSrc: "/assets/bluecard-images/partner-logos/Mosa logo.jpg",
    description: "Use code SUS20 for 20% off all products.",
    location: "Valid only at www.wearemosa.com",
    website: "https://wearemosa.com/?srsltid=AfmBOorLe6OPrpA1PMSqzv8JKXdnSz_OIz_n3xuQXFg5vdn7TyR-1viq",
  },
  {
    name: "Steve's Poke Bar",
    logoSrc: "/assets/bluecard-images/partner-logos/Steve_s Poke Bar Logo.png",
    description: "Use code SCHOOLSPB for $5 off first time mobile app orders.",
    location: "Valid at www.stevespokebar.ca",
    website: "https://www.stevespokebar.ca/",
  },
  {
    name: "Storagehotel",
    logoSrc: "/assets/bluecard-images/partner-logos/Storagehotel Logo - Green Small.png",
    description: "Use code SUS to save $50 and skip the Summer 2025 waitlist.",
    location: "Valid at www.storagehotel.ca",
    website: "https://www.storagehotel.ca/",
  },
  {
    name: "Wizeprep",
    logoSrc: "/assets/bluecard-images/partner-logos/wizeprep logo.png",
    description: "Get 15% off UBC-specific and MCAT courses.",
    location: "Valid at wizeprep.com",
    website: "https://www.wizeprep.com/",
  },
  {
    name: "Delhi 6 Indian Bistro",
    logoSrc: "/assets/bluecard-images/partner-logos/delhi.png",
    description: "Get 15% off all food items (not applicable to drinks).",
    location: "Valid at 1766 W 7th Ave, Vancouver, BC V6J 4T3",
    website: "https://www.delhi6.ca/",
  },
  {
    name: "Pearl Fever",
    logoSrc: "/assets/bluecard-images/partner-logos/pearl.png",
    description:
      "Get 10% off mobile app purchases using their mailing list: https://forms.gle/osEaqy5TrsLAKKiT7. Use SUSPF2025 for 10% off and milktea99 for $4.99 milk tea.",
    location: "Valid at 2182 Western Pkwy, Vancouver, BC V6T 1W6",
    website: "https://pearlfever.ca/",
  },
  {
    name: "Hullo Ferries",
    logoSrc: "/assets/bluecard-images/partner-logos/hullo.svg",
    description: "Use code UBCSUS10 for corporate rate.",
    location: "Valid at hullo.com",
    website: "https://hullo.com/",
  },
  {
    name: "MYST Asian Fusion",
    logoSrc: "/assets/bluecard-images/partner-logos/myst-asian-fusion-logo.jpg",
    description:
      "10% off all in-person meals,\n15% off event orders (50+ meals), and \n20% off event orders (100+ meals).",
    location:
      "Valid at MYST Asian Fusion @ 6400 Kingsway, Burnaby / MYST Asian Bistro @ 2028 W 41st Ave, Vancouver",
    website: "https://myst6400.com/",
  },
  {
    name: "Freshslice UBC Nest",
    logoSrc: "/assets/bluecard-images/partner-logos/freshslice.png",
    description:
      "$7.00 for 2XL Classic & Pop Combo,\n$7.90 for 2XL Feast & Pop Combo\n$4.90 for Cheesy Breads.",
    location: "Valid at UBC AMS Student Nest",
    website: "https://www.freshslice.com/",
  },
  {
    name: "Chickasta Bar",
    logoSrc: "/assets/bluecard-images/partner-logos/chickasta.png",
    description:
      "Get 10% off at Chickasta Grill.\nGet 15% off at House of Parantha.",
    location: "4443 W 10th Ave, Vancouver, BC V6R 2H8",
    website: "https://chickasta.com/",
  },
  {
    name: "Wren Cafe",
    logoSrc: "/assets/bluecard-images/partner-logos/wrenCafe.jpeg",
    description: "Use code WREN15 to get 15% off all in-store orders.",
    location: "Valid at 280 Nelson St, Vancouver, BC V6B 2E2",
    website: "https://wrencafe.ca/",
  },
  {
    name: "Hakumori",
    logoSrc: "/assets/bluecard-images/partner-logos/hakumori.webp",
    description: "Use code UBCSUS10 for 10% off storewide.",
    location: "Valid at www.hakumori.shop/",
    website: "http://www.hakumori.shop/",
  },
  {
    name: "Convivial Jewelry",
    logoSrc: "/assets/bluecard-images/partner-logos/convivial.png",
    description: "Use code SUS for 10% off online orders.",
    location: "Valid at www.convivial.ca/ ",
    website: "http://www.convivial.ca/",
  },
  {
    name: "DVS",
    logoSrc: "/assets/logos/DVS Black Logo (1).png",
    description:
      "Use code UBCBLUECARD25 to get 25% off 1-month unlimited membership, ongoing memberships, and Class Passes.",
    location: "Valid at Friday Party only and redeemable at the front desk.",
    website: "https://www.dancevancouver.ca/",
  },
];
